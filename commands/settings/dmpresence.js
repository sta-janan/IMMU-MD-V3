...
module.exports = {
    ...
    async execute({ socket, jid, args, number, isOwner, settings }) {
        ...
        if (!VALID.includes(val)) return sendStyled(socket, jid, { text: `📌 Usage: .dmpresence <${VALID.join('/')}>` }, { botName, number });
        settingsStore.set(number, 'dmPresence', val);
        await sendStyled(socket, jid, { text: `✅ DM presence set to *${val}*` }, { botName, number });
    }
};
