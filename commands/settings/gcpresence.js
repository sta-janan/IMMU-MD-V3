...
module.exports = {
    ...
    async execute({ socket, jid, args, number, isOwner, settings }) {
        ...
        if (!VALID.includes(val)) return sendStyled(socket, jid, { text: `📌 Usage: .gcpresence <${VALID.join('/')}>` }, { botName });
        settingsStore.set(number, 'gcPresence', val);
        await sendStyled(socket, jid, { text: `✅ Group presence set to *${val}*` }, { botName });
    }
};
